...
object ScalaSigReader {
  ...
  def findClass(clazz: Class[?]): ClassSymbol = {
  ...
  def findCompanionObject(clazz: Class[?]): ClassSymbol = {
    val sig = findScalaSig(clazz).getOrElse(fail("Can't find ScalaSig for " + clazz))
  ...
  private def findField(clazz: Class[?], name: String): Option[MethodSymbol] = findField(findClass(clazz), name)
  ...
  def findScalaSig(clazz: Class[?]): Option[ScalaSig] = try {
