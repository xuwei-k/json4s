...
object ScalaType {
  ...
  private[json4s] val ListObject: ScalaType =
    new ScalaType(Manifest.classType(
      classOf[List[_]],
      Manifest.classType(classOf[java.lang.Object])
    ))

  private[json4s] val Object: ScalaType =
    new ScalaType(Manifest.classType(classOf[java.lang.Object]))

  private[json4s] val MapStringObject: ScalaType =
    new ScalaType(Manifest.classType(
      classOf[Map[_, _]],
      Manifest.classType(classOf[String]),
      Manifest.classType(classOf[Object])
    ))
